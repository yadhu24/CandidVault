...
2. **Worker** — ⚠️ **cannot run on Vercel** (it's a long-running poll loop, not a
   request handler). Run `npm run worker` on an always-on host (Railway / Render /
   Fly / a small VM) with the same `DATABASE_URL`, `R2_*`, and
   `UPLOAD_SIGNING_SECRET`. Put `ffmpeg`/`ffprobe` on PATH for video posters
   (optional — it falls back gracefully).
